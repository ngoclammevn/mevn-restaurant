...
Desktop giữ cùng bốn nhóm navigation với mobile nhưng hiển thị trong top bar:

- Hôm nay
- Đăng menu
- Đơn của tôi
- Quản lý
- Avatar mở Hồ sơ, changelog và đăng xuất

Từ breakpoint `1080px`, content shell mở rộng tới khoảng `1120–1200px`; không giữ giới hạn
`760px` cho các màn hình cần khai thác chiều ngang.

`MenuPage` dùng hai cột:

- Cột trái `minmax(0, 1.7fr)`: hero, ảnh menu gốc, category và danh sách món.
- Cột phải rộng khoảng `320–360px`: order summary sticky, ghi chú, Đặt hộ, tổng tiền và CTA.
- Khoảng cách giữa hai cột là `24–32px`.
- Order rail sticky nằm dưới app bar và không che nội dung khi zoom trình duyệt.

Desktop không dùng bottom sticky CTA vì order rail đã luôn hiện. Từ `601–1079px`, layout trở về
một cột và dùng sticky CTA như mobile/tablet.

Các màn hình desktop khác:

- Hôm nay dùng menu grid hai cột khi đủ rộng.
- Quản lý dùng tabs Menu của tôi/Thu tiền, summary ở đầu và danh sách rộng phía dưới.
- Form Đăng menu và Hồ sơ giữ measure khoảng `680–760px` để dễ đọc.
- OCR preview của Đăng menu được mở hai cột: ảnh gốc và editor.
...
- Ảnh menu gốc luôn nằm trước kết quả OCR để người dùng đối chiếu.
...
Ảnh là file người đăng tải lên trước khi OCR nên không được giả định kích thước hoặc tỉ lệ:

- Không dùng fixed aspect ratio và không crop ảnh.
- Giữ tỉ lệ tự nhiên với `object-fit: contain`.
- Reference panel căn giữa ảnh trên nền trung tính.
- Ảnh dùng `max-width: 100%`, `width: auto`, `height: auto` và
  `max-height: min(55vh, 560px)` trên desktop.
- Không upscale ảnh nhỏ vượt quá kích thước tự nhiên.
- Ảnh quá cao được thu nhỏ để vẫn thấy trọn nội dung; click mở lightbox ở độ phân giải gốc.
- Lightbox hỗ trợ zoom, đóng bằng Escape và trả focus về ảnh.
- Nếu không có `image_url`, không render placeholder; nội dung phía dưới dồn lên.
- Nếu OCR lỗi hoặc menu không có cấu trúc, ảnh gốc vẫn là nội dung chính trước phần text tự do.

Ảnh lỗi tải hiển thị trạng thái lỗi gọn cùng nút Thử lại; danh sách món/text không bị chặn.
...
- `MenuImageReference.vue` — ảnh gốc responsive, loading/error và lightbox.
...
- QA bắt buộc ở 360px, 390px, 430px, 1080px, 1280px và 1440px.
- Ảnh menu phải được QA với ảnh dọc, ngang, rất dài, nhỏ hơn container và lỗi tải.
...
- MenuHero, MenuImageReference, DishList và StickyOrderBar.
- Desktop two-column shell với sticky order rail.
...
- Desktop tận dụng shell rộng nhưng form đọc dài không vượt quá measure `760px`.
- Ảnh menu gốc không bị crop ở mọi tỉ lệ được kiểm thử.
...
- MenuPage hai cột ở 1080/1280/1440px; order rail sticky không che footer hoặc dialog.
- Ảnh dọc/ngang/rất dài giữ trọn nội dung và mở lightbox đúng.
- Ảnh lỗi không chặn dish list, text menu hoặc order form.
- Hôm nay hiển thị grid hai cột; Manage tabs và top navigation dùng được bằng keyboard.
- Browser zoom 200% chuyển layout an toàn về một cột khi không còn đủ chiều ngang.
