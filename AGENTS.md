...
- **Đặt món:** plain text menu → free text; menu có cấu trúc OCR (`dishes[]` hợp lệ) → bắt buộc chọn từ MenuBoard (không cho nhập tay), ghi chú vẫn free text. Tính tiền tự động chỉ hoạt động khi menu có cấu trúc và tên món khớp chính xác.
